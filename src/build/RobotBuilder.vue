    ...
    <div class="top-row">
      <div  :class="[salesBorderClass,'top','part']">
        <div class="robot-name">
          <p>{{selectedRobot.head.title}}
            <span class="sale" v-show="selectedRobot.head.onSale"
             :style="{color: 'red', fontWeight: 'bold'}">on Sale</span>
          </p>
        </div>
        <PartSelector
...
export default {
  ...
  computed: {
    ...
    headSyle() {
      return {
        border: this.selectedRobot.head.onSale ? '3px solid red' : '3px solid yellow'
      };
    },
    salesBorderClass() {
